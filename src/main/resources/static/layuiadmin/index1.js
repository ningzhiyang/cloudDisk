      ...
      ,success: function() {
        $.ajax({
          ...
          success: function(res){
            // 更多传入参数及其具体意义请查看文档
            alert(res.data.list)
            authtree.listConvert(res.data.list,opt);
            var selectList = authtree.treeConvertSelect(authtree.listConvert(res.data.list,data.checkedId), {
              childKey: 'list',
...
layui.use(['jquery', 'authtree', 'form', 'layer'], function(){
  ...
  data['id'] = $("#agId").val();
  if($("#agId").val()>0){
    var url = "/pay/group/testjson"
  }else{
    var url ="/pay/group/treeAll";
  }
  data['auType'] =2;
  // 初始化
  $.ajax({
  ...
  form.on('submit(LAY-auth-tree-submit)', function(obj){
    ...
    console.log('Choosed authids is', authids);
     obj.field.agAuth = authids.toString();
    if($("#agId").val()>0){
      $.ajax({
        url: '/pay/group/update',
        traditional : true,//数组**,
        dataType: 'json',
        type:'post',
        data: obj.field,
