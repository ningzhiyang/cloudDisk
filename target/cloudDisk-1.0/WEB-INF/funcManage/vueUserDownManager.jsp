...
<script>
    layui.use(['table', 'element', 'form', 'layer'], function () {
        ...
        table.render({
            elem: '#LAY_table_user'
            , url: '<%=request.getContextPath()%>/bg/vue_user_down_manager/vue_user_down_manager_json'
            , toolbar: '#tools'
            , title: '用户数据表'
            , totalRow: true
            , cols: [[
                ...
                , {field: 'fname', title: '文件名称',
                    templet : function(row) {
                        return row.vueFile.fname;
                    }}
                , {field: 'fext', title: '文件类型',
                    templet : function(row) {
                        return row.vueFile.fext;
                    }}
                , {field: 'fpath', title: '文件路径',
                    templet : function(row) {
                        return row.vueFile.fpath;
                    }}
                , {field: 'fsize', title: '文件大小',
                    templet : function(row) {
                        return row.vueFile.fsize;
                    }}
                , {field: 'vaccount', title: '下载账户',
                    templet : function(row) {
                        return row.vueUser.vaccount;
                    }}
                , {field: 'vphone', title: '下载手机号', align: 'center', fixed: 'right',
                    templet : function(row) {
                        return row.vueUser.vphone;
                    }}
