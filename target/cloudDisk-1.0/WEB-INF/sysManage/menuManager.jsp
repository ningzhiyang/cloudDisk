            ...
            <div class="layui-form-item demoTable">
                			  <div class="layui-inline">
                				<div class="layui-input-inline">
                				  <input type="text" name="name" id="demoReload" placeholder="请输入菜单名称" autocomplete="off" class="layui-input">
                				</div>
                			  </div>
                			  <div class="layui-inline">
...
<script>
    layui.use(['table', 'element', 'form', 'layer'], function () {
        ...
        var statusTpl = function (d) {
            if (d.mstatus === true) {
                return '<input type="checkbox" id="status" value="' + d.mid + '" lay-filter="stat" lay-skin="switch" lay-text="开启|禁用" checked> ';
            } else {
                return '<input type="checkbox" id="status" value="' + d.mid + '" lay-filter="stat" lay-skin="switch" lay-text="开启|禁用"> ';
            }
        };

        table.render({
            elem: '#LAY_table_user'
            , url: '<%=request.getContextPath()%>/bg/menu_manager/menu_manager_json'
            , toolbar: '#tools'
            , title: '用户数据表'
            , totalRow: true
            , cols: [[
                ...
                // , {field: 'micon', title: '菜单图标'}
                , {field: 'mname', title: '菜单名称'}
                , {field: 'murl', title: '菜单url路径'}
                , {field: 'mstatus', title: '菜单状态', align: 'center', fixed: 'right', templet: statusTpl}
        ...
        form.on('switch(stat)', function (data) {
            ...
            if (x === true) {
                ...
                $.ajax({
                    url: "<%=request.getContextPath()%>/bg/menu_manager/switch",
                    data: data1,
                    type: "Post",
                    dataType: "json",
        ...
        table.on('tool(LAY_table_user)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.ajax({
                        url: "<%=request.getContextPath()%>/bg/menu_manager/delete",
                        data: data1,
                        type: "Post",
                        dataType: "json",
                        success: function (data) {
                ...
                layer.open({
                    ...
                    shade: 0.6,
                    area: ['780px', '500px'],
                    content: '<%=request.getContextPath()%>/bg/menu_manager/update?id='+obj.data.mid
                });
        ...
        table.on('toolbar(LAY_table_user)', function (obj) {
            if (obj.event === "add") {
                layer.open({
                    ...
                    shade: 0.6,
                    area: ['780px', '500px'],
                    content: '<%=request.getContextPath()%>/bg/menu_manager/insert'
                });
        ...
        var $ = layui.$, active = {
            reload: function () {
                ...
                table.reload('testReload', {
                    ...
                    , url: "<%=request.getContextPath()%>/bg/menu_manager/findListInfo"
                    , method: "post"
                }, 'data');
            }
        };
