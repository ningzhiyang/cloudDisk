                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <c:forEach items="${allmenu}" var="menu">
                            <input type="checkbox" name="rGroup" title="${menu.MName}" value="${menu.MId}"
                                <c:forEach items="${sysMenus}" var="menu1">
                                    <c:if test="${menu.MId == menu1.MId}">
                                        checked
                                    </c:if>
                                </c:forEach>
                            >
...
<script>
    layui.config({
        ...
        form.on('submit(component-form-demo)', function (data) {
            var group = $("input:checkbox[name='rGroup']:checked").map(function(index,elem) {
                return $(elem).val();
            }).get().join(',');
            data.field.rGroup=group;
            // console.log(data.field)
            $.post("<%=request.getContextPath()%>/bg/role_manager/to_key", data.field, function (data) {
                // 获取 session
