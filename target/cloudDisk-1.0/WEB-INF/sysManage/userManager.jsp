...
<script>
    layui.use(['table', 'element', 'form', 'layer'], function () {
        ...
        table.render({
            elem: '#LAY_table_user'
            , url: '<%=request.getContextPath()%>/bg/user_manager/user_manager_json'
            , toolbar: '#tools'
            , title: '用户数据表'
            , totalRow: true
            , cols: [[
        ...
        table.on('tool(LAY_table_user)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.ajax({
                        url: "<%=request.getContextPath()%>/bg/user_manager/delete",
                        data: data1,
                        type: "Post",
                        dataType: "json",
                        success: function (data) {
                ...
                layer.open({
                    ...
                    shade: 0.6,
                    area: ['780px', '500px'],
                    content: '<%=request.getContextPath()%>/bg/user_manager/update?id='+obj.data.uid
                });
        ...
        table.on('toolbar(LAY_table_user)', function (obj) {
            if (obj.event === "add") {
                layer.open({
                    ...
                    shade: 0.6,
                    area: ['780px', '500px'],
                    content: '<%=request.getContextPath()%>/bg/user_manager/insert'
                });
